...
function createSessionId(remotePort) {
	if (sessionIdCounter <= 9999999999) {
		return process.pid + '' + remotePort + '' + (sessionIdCounter++);
	} else {
		sessionIdCounter = 1;
		return process.pid + '' + remotePort + '' + (sessionIdCounter++);
	}
...
function WebSocketConnection(socket, extensions, protocol, maskOutgoingPackets, config) {
    ...
    this.sessionId = createSessionId(socket.remotePort);
